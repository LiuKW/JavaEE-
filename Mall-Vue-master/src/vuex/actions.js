...
export const loadSeckillsInfo = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    axios({
      method: 'get',
      url: 'http://kingwait.com/supermarket/good/recommend'
    }).then(response => {
      console.log(response.data); //请求成功返回的数据
      if (response.data.code == 10000) {
        data = response.data.recommends
...
export const loadGoodsInfo = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    axios({
      method: 'get',
      url: 'http://kingwait.com/supermarket/good/'+id
    }).then(response => {
      console.log(response.data); //请求成功返回的数据
      if (response.data.code == 10000) {
        console.log(response.data.good);

        var GoodData = response.data.good;
        commit('SET_GOODS_INFO', GoodData);
    ...
    setTimeout(() => {commit('SET_LOAD_STATUS', false)}, 5000);  //超时
...
export const loadGoodsList = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    if(id == 0 || id == undefined) {
      requestUrl = 'http://kingwait.com/supermarket/good/pages'
    } else {
      requestUrl = 'http://kingwait.com/supermarket/good/pages/' + id
    }
    ...
    axios({
      ...
      console.log(response.data); //请求成功返回的数据
      if (response.data.code == 10000) {
        console.log(response.data.goods.records);

        var GoodData = response.data.goods.records;
        commit('SET_GOODS_LIST', GoodData);
        commit('SET_LOAD_STATUS', false);
      } else {
...
export const loadAddress = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    axios({
      method: 'get',
      url: 'http://kingwait.com/supermarket/user-address/'+userID,
      data: {
        a: 1
      }
    }).then(response => {
...
export const login = ({
  ...
  return new Promise((resolve, reject) => {
    axios({
      method: 'post',
      url: 'http://kingwait.com/supermarket/user/login',
      data: {
