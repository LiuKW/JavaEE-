  ...
  <div>
    ...
    <div class="nav-body">
      ...
      <div class="nav-side" ref="navSide">
        <ul v-for="(item,index) in categoryItem" :key="index">
          <li>
            <span class="nav-side-item" v-on:click="showCategory(item.catId)">{{item.catName}}</span>
          </li>
        </ul>
      </div>
      <div class="nav-content">
        <img src="../../../static/img/background.jpg" style="width: 860px;" />
        <!-- 幻灯片 -->
        <!--<div>
...
export default {
  ...
  methods: {
      ...
      this.$axios({
        ...
        if(response.data.code == 10000) {
          ...
          if(response.data.categories) {
            that.categoryItem = response.data.categories;
          } else {
            that.categoryItem = [];
          }
          console.log(that.categoryItem);
        }
      }).catch(
    ...
    showCategory (catId) {
      console.log(catId);
      this.$router.push({ path: '/goodsList' , query: { id: catId } });
    }
  },
  created () {
    this.getCategory();
