<template>
  <div>
    ...
    <div class="container" v-if="showItem">
      ...
      <i-button type="primary" @click="reload">切换</i-button>
      <!-- 商品展示容器 -->
      <div class="goods-box">
        <div class="goods-list-box">
          ...
          <div class="goods-list">
            <div class="goods-show-info" v-for="(item, index) in orderGoodsList" :key="index">
              <router-link :to="{ path: '/goodsDetail', query: { id: item.goodId } }">
              <div class="goods-show-img">
                <img :src="item.goodMainimageurl" style="width: 220px;height: 220px;" />
              </div>
              <div class="goods-show-price">
                <span>
                  <Icon type="social-yen text-danger"></Icon>
                  <span class="seckill-price text-danger">{{item.goodPrice.toFixed(2)}}</span>
                </span>
              </div>
              ...
              <div class="goods-show-num">
                已有<span>{{item.goodSale}}</span>人购买
              </div>
              </router-link>
            </div>
          </div>
  ...
  methods: {
    ...
    reload () {
      ...
      if (that.$route.query.id) {
        that.catId = that.$route.query.id;
      }
      that.loadGoodsList(that.catId);
    }
  },
  created () {
    var that = this;
